...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .invoice-paper{width:100%;max-width:860px;margin:auto;background:#ffffff!important;color:#111!important;padding:28px 32px;box-sizing:border-box;font-family:Arial,sans-serif;font-size:13px;border:1px solid #ddd;border-radius:8px}
  ...
  .inv-table th{background:#f5f5f5!important;color:#333!important;padding:8px 10px;font-size:11px;font-weight:700;text-transform:uppercase;border-bottom:2px solid #ddd;border-top:1px solid #ddd;text-align:left;white-space:nowrap}
  ...
  @media print{
    *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
    ...
    .sidebar,.topbar,.btn,.section-header,.alert,.search-wrap{display:none!important}
    .content>*{display:none!important}
    .invoice-paper,#ticket-print-area{display:block!important}
    .invoice-paper{width:100%;padding:10mm;box-shadow:none;border:none;background:#fff!important;color:#111!important}
    .inv-company-name,.inv-no{color:#c48b00!important}
    .inv-table th{background:#f4f4f4!important;color:#333!important}
...
function Dashboard({ cargo, tickets, bookings }) {
  ...
  const bars = [{ label: "Cargo", val: totalCargoRev, color: "#f0a500" }, { label: "Tickets", val: totalTicketRev, color: "#58a6ff" }, { label: "Bookings", val: totalBookingRev, color: "#a371f7" }];
  return (
    <div>
      <div className="sheets-banner">
        <div className="sheets-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="#3fb950"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg></div>
        <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2 }}>Perwaani Operations · perwaani2023@gmail.com</div><div style={{ fontSize: 12, color: "var(--muted)" }}>Data saved locally via localStorage.</div></div>
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        ...
        <div className="card kpi green"><div className="kpi-icon"><Icon name="cargo" size={40} /></div><div className="kpi-label">Cargo Revenue</div><div className="kpi-val">SSP {fmt(totalCargoRev)}</div><div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>{cargo.length} shipments</div></div>
        <div className="card kpi blue"><div className="kpi-icon"><Icon name="plane" size={40} /></div><div className="kpi-label">Ticket Revenue</div><div className="kpi-val">SSP {fmt(totalTicketRev)}</div><div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>{tickets.length} passengers</div></div>
        <div className="card kpi purple"><div className="kpi-icon"><Icon name="booking" size={40} /></div><div className="kpi-label">Booking Revenue</div><div className="kpi-val">SSP {fmt(totalBookingRev)}</div><div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>{bookings.length} bookings</div></div>
      </div>
      <div className="grid-2" style={{ marginBottom: 20 }}>
        ...
        <div className="card">
          ...
          {Object.keys(payBreakdown).length === 0 ? <div style={{ color: "var(--muted)", fontSize: 13 }}>No data yet.</div> : Object.entries(payBreakdown).map(([k, v]) => (<div className="stat-row" key={k}><PayPill method={k} /><span className="stat-num">SSP {fmt(v)}</span></div>))}
          <hr className="divider" />
          <div className="section-title" style={{ marginBottom: 12 }}>Cargo Item Types</div>
          {Object.keys(itemBreakdown).length === 0 ? <div style={{ color: "var(--muted)", fontSize: 13 }}>No data yet.</div> : Object.entries(itemBreakdown).slice(0, 5).map(([k, v]) => (<div className="stat-row" key={k}><span className="stat-name">{k}</span><span className="stat-num">{v} shipment{v > 1 ? "s" : ""}</span></div>))}
        </div>
      </div>
      <div className="card">
        <div className="section-title" style={{ marginBottom: 14 }}>Recent Activity</div>
        {recentAll.length === 0 ? <div className="empty"><Icon name="dashboard" size={40} /><h3>No activity yet</h3><p>Start by adding cargo, tickets, or bookings.</p></div> : (
          <div className="table-wrap"><table><thead><tr><th>Type</th><th>ID</th><th>Name</th><th>Date</th><th>Amount (SSP)</th></tr></thead><tbody>{recentAll.map((r, i) => (<tr key={i}><td><span className={`badge ${r.color === "accent" ? "badge-amber" : r.color === "blue" ? "badge-blue" : "badge-purple"}`}>{r.type}</span></td><td className="mono">{r.id || "—"}</td><td>{r.name}</td><td style={{ color: "var(--muted)", fontSize: 12 }}>{r.date || "—"}</td><td className="mono" style={{ color: "var(--accent)" }}>{fmt(r.amount)}</td></tr>))}</tbody></table></div>
        )}
      </div>
    </div>
  );
...
function CargoRegister({ data, setData, toast }) {
  ...
  const set = (k, v) => setForm(f => { const nf = { ...f, [k]: v }; if (k === "unitPrice" || k === "qty") { const p = parseFloat(k === "unitPrice" ? v : nf.unitPrice) || 0; const q = parseFloat(k === "qty" ? v : nf.qty) || 0; nf.amount = p * q || ""; } return nf; });
  ...
  const submit = async () => {
    ...
    const updated = editing ? data.map(r => r.id === editing ? { ...form, id: editing } : r) : [...data, { ...form, id: `CRG-${String(data.length + 1).padStart(3, "0")}` }];
    toast(editing ? "Updated ✓" : "Saved ✓", "success");
    setData(updated); await saveData(KEYS.cargo, updated); setShowForm(false); setEditing(null); setForm(emptyForm); setErrors({});
  };
  const del = async (id) => { const u = data.filter(r => r.id !== id); setData(u); await saveData(KEYS.cargo, u); toast("Deleted", "error"); };
      ...
      {showForm && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal">
            ...
            <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setShowForm(false)}>Cancel</button><button className="btn btn-primary" onClick={submit}><Icon name="check" size={15} />{editing ? "Update" : "Save"}</button></div>
          </div>
        </div>
      )}
    </div>
...
function Ticketing({ data, setData, toast }) {
  ...
  const submit = async () => {
    ...
    const updated = editing ? data.map(r => r.id === editing ? { ...form, id: editing } : r) : [...data, { ...form, id: uid("TKT") }];
    toast(editing ? "Updated ✓" : "Ticket saved ✓", "success");
    setData(updated); await saveData(KEYS.tickets, updated); setShowForm(false); setEditing(null); setForm({ ...emptyForm, ticketNo: nextTicket + 1 });
  };
  ...
  const filtered = data.filter(r => (r.passengerName || "").toLowerCase().includes(search.toLowerCase()) || (r.ticketNo + "").includes(search));
    ...
    <div>
      ...
      {filtered.length === 0 ? <div className="card"><div className="empty"><Icon name="ticket" size={40} /><h3>No tickets yet</h3><p>Issue the first travel ticket.</p></div></div> : (
        <div className="card"><div className="table-wrap"><table>
          ...
          <tbody>
            {filtered.map(r => (<tr key={r.id}><td className="mono" style={{ color: "var(--accent)", fontWeight: 700 }}>{r.ticketNo}</td><td style={{ fontSize: 12 }}>{r.date || "—"}</td><td style={{ fontWeight: 600 }}>{r.passengerName || "—"}</td><td><div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12 }}><span style={{ color: "var(--blue)" }}>{r.from || "—"}</span><span style={{ color: "var(--muted)" }}>→</span><span style={{ color: "var(--purple)" }}>{r.to || "—"}</span></div></td><td className="mono" style={{ fontSize: 12 }}>{r.flightNo || "—"}</td><td style={{ fontSize: 12 }}>{r.departureTime || "—"}</td><td className="mono" style={{ color: "var(--green)", fontWeight: 700 }}>{fmt(r.fees)}</td><td><StatusBadge status={r.paymentStatus} /></td><td><div style={{ display: "flex", gap: 4 }}><button className="btn btn-ghost btn-sm" onClick={() => printTicket(r)}><Icon name="print" size={13} /></button><button className="btn btn-ghost btn-sm" onClick={() => openEdit(r)}><Icon name="edit" size={13} /></button><button className="btn btn-danger btn-sm" onClick={() => del(r.id)}><Icon name="trash" size={13} /></button></div></td></tr>))}
      ...
      {showForm && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal">
            ...
            <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setShowForm(false)}>Cancel</button><button className="btn btn-primary" onClick={submit}><Icon name="check" size={15} />{editing ? "Update" : "Issue"}</button></div>
          </div>
        </div>
      )}
      {printTkt && (
        <div style={{ display: "none" }} id="ticket-print-area">
          <div className="ticket-paper">
            <div className="tkt-header"><div><div className="tkt-company">PERWAANI</div><div className="tkt-sub">General Trading & Investment Co. Ltd · Juba Airport Road, South Sudan</div></div><div style={{ textAlign: "right" }}><div style={{ fontSize: 11, opacity: 0.8 }}>BOARDING PASS</div><div className="tkt-no">#{printTkt.ticketNo}</div></div></div>
            <div className="tkt-route"><div><div className="tkt-airport">{printTkt.from || "—"}</div><div className="tkt-city">Origin</div></div><div className="tkt-arrow">✈ ──────</div><div><div className="tkt-airport">{printTkt.to || "—"}</div><div className="tkt-city">Destination</div></div></div>
            <div className="tkt-body">
              <div className="tkt-row"><div className="tkt-cell"><div className="tkt-cell-label">Passenger</div><div className="tkt-cell-val">{printTkt.passengerName || "—"}</div></div><div className="tkt-cell"><div className="tkt-cell-label">Phone</div><div className="tkt-cell-val">{printTkt.phone || "—"}</div></div><div className="tkt-cell"><div className="tkt-cell-label">Date</div><div className="tkt-cell-val">{printTkt.date || "—"}</div></div></div>
              <div className="tkt-row"><div className="tkt-cell"><div className="tkt-cell-label">Flight</div><div className="tkt-cell-val">{printTkt.flightNo || "—"}</div></div><div className="tkt-cell"><div className="tkt-cell-label">Departure</div><div className="tkt-cell-val">{printTkt.departureTime || "—"}</div></div><div className="tkt-cell"><div className="tkt-cell-label">Arrival</div><div className="tkt-cell-val">{printTkt.arrivalTime || "—"}</div></div></div>
            </div>
            <div className="tkt-amount"><div><div className="tkt-amount-label">SERVICE FEE</div></div><div className="tkt-amount-val">SSP {fmt(printTkt.fees)}</div></div>
            <div className="tkt-footer">Perwaani General Trading & Investment Co. Ltd · +211 (0) 920 000 149 · perwaani2023@gmail.com</div>
          </div>
        </div>
      )}
    </div>
...
function Bookings({ data, setData, toast }) {
  ...
  const submit = async () => {
    if (!validate()) { toast("Please fix errors", "error"); return; }
    const updated = editing ? data.map(r => r.id === editing ? { ...form, id: editing } : r) : [...data, { ...form, id: uid("B"), bookingId: `BKG-${String(data.length + 1).padStart(4, "0")}`, total: (parseFloat(form.fare) || 0) + (parseFloat(form.taxes) || 0) }];
    toast(editing ? "Updated ✓" : "Booking confirmed ✓", "success");
    setData(updated); await saveData(KEYS.bookings, updated); setShowForm(false); setEditing(null); setForm(emptyForm); setErrors({});
  };
  const del = async (id) => { const u = data.filter(r => r.id !== id); setData(u); await saveData(KEYS.bookings, u); toast("Deleted", "error"); };
  const openEdit = (r) => { setForm(r); setEditing(r.id); setShowForm(true); setErrors({}); };
  const filtered = data.filter(r => (r.passengerName || "").toLowerCase().includes(search.toLowerCase()) || (r.bookingId || "").includes(search));
      ...
      {showForm && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal">
            ...
            <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setShowForm(false)}>Cancel</button><button className="btn btn-primary" onClick={submit}><Icon name="check" size={15} />{editing ? "Update" : "Confirm"}</button></div>
          </div>
        </div>
      )}
    </div>
          ...
          <div style={{ overflowY: "auto" }}>
            <div className="invoice-paper">
              ...
              <table className="inv-table">
                ...
                <tfoot><tr><td colSpan={6} style={{ textAlign: "right", color: "#555" }}>Subtotal</td><td style={{ textAlign: "right" }}>{fmt(preview.subtotal)}</td></tr><tr><td colSpan={6} style={{ textAlign: "right", color: "#555" }}>Tax ({preview.taxPct}%)</td><td style={{ textAlign: "right" }}>{fmt(preview.tax)}</td></tr><tr><td colSpan={6} style={{ textAlign: "right", fontWeight: 700 }}>GRAND TOTAL (SSP)</td><td style={{ textAlign: "right", fontWeight: 800, color: "#c48b00" }}>{fmt(preview.grand)}</td></tr></tfoot>
              </table>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginTop: 12 }}><button className="btn btn-primary" onClick={() => window.print()}><Icon name="print" size={15} />Print Invoice</button></div>
          </div>
        ) : (
...
function Reports({ cargo, tickets, bookings }) {
  ...
  const target = 20000000;
  const kpis = [{ name: "Cargo Revenue (SSP)", actual: cargoRev, target: 15000000, owner: "Operations" }, { name: "Ticket Revenue (SSP)", actual: ticketRev, target: 5000000, owner: "Ticketing" }, { name: "Booking Revenue (SSP)", actual: bookingRev, target: 3000000, owner: "Booking" }, { name: "Total Transactions", actual: txns, target: 80, owner: "All Depts" }, { name: "Combined Revenue (SSP)", actual: total, target, owner: "Finance" }];
  const payBreak = {};
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div className="table-wrap"><table>
          <thead><tr><th>Metric</th><th>Cargo</th><th>Ticketing</th><th>Booking</th><th>Total</th><th>Target</th><th>Variance</th><th>% Achieved</th><th>Status</th></tr></thead>
          <tbody>
            <tr><td style={{ fontWeight: 600 }}>Revenue (SSP)</td><td className="mono">{fmt(cargoRev)}</td><td className="mono">{fmt(ticketRev)}</td><td className="mono">{fmt(bookingRev)}</td><td className="mono" style={{ color: "var(--accent)", fontWeight: 700 }}>{fmt(total)}</td><td className="mono">{fmt(target)}</td><td className="mono" style={{ color: total >= target ? "var(--green)" : "var(--red)" }}>{fmt(total - target)}</td><td className="mono">{((total / target) * 100).toFixed(1)}%</td><td><StatusBadge status={total >= target ? "Confirmed" : "Pending"} /></td></tr>
        ...
        <div className="main">
          <div className="topbar">
            <div className="topbar-left">
              ...
              <div><div className="page-title">{titles[page]}</div><div className="page-sub">Perwaani General Trading & Investment Co. Ltd · Juba, South Sudan</div></div>
            </div>
          ...
          <div className="content">
            {loading ? <div style={{ textAlign: "center", padding: "80px 20px", color: "var(--muted)" }}>Loading…</div> : (
              <>
